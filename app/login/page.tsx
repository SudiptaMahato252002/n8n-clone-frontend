      ...
      <div className='flex items-center gap-16'>
        ...
        <aside className='flex flex-1 items-center relative'>
            <form action="" className='flex flex-col justify-center border rounded-lg p-6 w-full max-w-sm bg-white shadow-sm' >
              ...
              <Input label='Email' onChange={(e)=>{setEmail(e.target.value)}} type='text' placeholder='Your Email'></Input>
              <Button type='submit' 
              disabled={isDisabled} 
              className={cn('mt-2 py-2 font-medium rounded-md transition',
                isDisabled?'bg-gray-300 text-gray-600 cursor-not-allowed hover:bg-gray-300'
                :'bg-orange-500 text-white hover:bg-ornage-600'
              )}>Contniue</Button>

              <div className="text-center text-sm text-gray-600">
                <p>Don't have an account?</p>
                <Link href={'/signup'} className="text-blue-500 font-medium hover:underline">Signup</Link>
              </div>
